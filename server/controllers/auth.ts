...
async function registerUser(req, res) {
    const { email, password, name, surname }: ICreateUser = await req.body;
    ...
    connection.query(
        ...
        VALUES (?, ?, ?, ?, ?)
    `,
        [email, hashPassword, dateNow, name, surname],
        (err, result) => {
            if (!err) {
...
function updateToken(email: string, token: string) {
    connection.query(
        `
...
async function loginUser(req, res) {
    const { email, password }: ICreateUser = await req.body;
